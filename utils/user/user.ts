import getLocalStorageItemKeys from '../storage/local-storage'
import { Order } from '@gelatonetwork/limit-orders-lib'
import { get, set, clear } from 'local-storage'
import orderBy from 'lodash/orderBy'
import { connectorLocalStorageKey, Providers } from '../web3/web3React'
...
export const clearUserStates = () => {
  ...
  if (window?.localStorage?.getItem('walletconnect')) {
    (Providers.walletconnect as WalletConnectConnector).close(),
    (Providers.walletconnect as WalletConnectConnector).walletConnectProvider = null
  }
